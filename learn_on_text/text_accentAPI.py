...
MAXLEN = 40
CHARS = ["'", '-', '_', 'а', 'б', 'в', 'г', 'д', 'е', 'ж', 'з', 'и', 'й', 'к','л', 'м', 'н', 'о', 'п', 'р', 'с', 'т',
...
def predict(model_for_prediction, w):
    ...
    w = w[w.index('_') + 1:]
    index = len(w) - MAXLEN + index
    #print(preds)
    #print('max_value in preds is %s with index %s' % (max_value, index))
    if index > len(w)-1:
        print('no %s-th letter in %s' % (index + 1, w))
    else:
        acc_word = w[:index + 1] + '\'' + w[index + 1:]
...
def main(list_of_strings):
    ...
    for string in list_of_strings:
        ...
        for plusword in pluswords:
            if not bool(re.search(REG, plusword)):
                accented_phrase.append(plusword)
            else:
                accented_phrase.append(predict(loaded_model, plusword))
...
if __name__ == "__main__":
    result = main(
        ['ну что, потестим эту систему?', 'посмотрим, работают ли мои изменения.'])

    for line in result:
        print(line)
        line_clean = line.replace('_', '').split()
        for element in line_clean:
            print(element)
